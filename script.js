document.getElementById("analyzeBtn").addEventListener("click", async () => {
    ...
    try {
        ...
        // Yêu cầu: Đánh giá theo thang điểm ảnh cung cấp
        ...
        // Yêu cầu: Giãn cách dòng thoáng mắt
        // Yêu cầu: Nhãn in đậm
        function renderBar(s, label) {
            let score = Math.max(0, Math.min(10, parseInt(s) || 0)); // Giữ thang 0-10, chặn âm
            return `
        ...
        res.innerHTML = `
            ...
            <div class="card" style="grid-column: span 2;"><h3>Phần 3: Phân tích & Giải pháp</h3>
                <p>${obj.analysis.chuyen_sau}</p>
                <ul style="padding-left: 20px;">${obj.analysis.giai_phap.map(g => `<li>${g}</li>`).join('')}</ul>
